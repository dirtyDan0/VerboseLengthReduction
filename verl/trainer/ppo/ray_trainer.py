...
from verl.utils.dumper import Dumper
...
class RayPPOTrainer(object):
    ...
    def __init__(self,
        ...
        self.dumper = Dumper(tokenizer=self.tokenizer,
                             file_path=self.config.trainer.dump_path,
                             exp_name=self.config.trainer.experiment_name) if self.config.trainer.dump_path else None
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            self.use_critic = True
                ...
                AdvantageEstimator.RLOO, AdvantageEstimator.REINFORCE_PLUS_PLUS_BASELINE, AdvantageEstimator.REINFORCE
        ]:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if self.dumper:
                self.dumper.dump(data=test_batch, step=self.global_steps, split="val")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # Dumping
                    if self.dumper:
                        self.dumper.dump(data=batch, step=self.global_steps, split="train")
