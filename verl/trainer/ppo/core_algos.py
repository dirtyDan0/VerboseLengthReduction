...
def compute_reinforce_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor):
    """
    Compute advantage for REINFORCE (for ppo-like loss, \lambda=1). 
    OUTCOME ONLY.
    Args:
        token_level_rewards: `(torch.Tensor)`
            shape: (bs, response_length)
    ...
    with torch.no_grad():
        returns = (token_level_rewards * response_mask).sum(dim=-1, keepdim=True)
...
def compute_policy_loss_orm(old_log_prob,
                            log_prob,
                            advantages,
                            response_mask,
                            cliprange=None,
    ...
    Args:
        ...
        log_prob: `(torch.Tensor)`
            shape: (bs, response_length)
        advantages: `(torch.Tensor)`
            shape: compatible with (bs,1) or (bs, response_length)
        response_mask: `(torch.Tensor)`
            shape: (bs, response_length)
        cliprange: (float)
            The clip range used in PPO. See https://arxiv.org/abs/1707.06347
    ...
    ppo_kl = verl_F.masked_mean(-log_diff, response_mask)

    # (bs,num_actions)
    ratio = torch.exp(log_diff)
    clip_ratio_higher = torch.clamp(ratio, 1 - cliprange_low, 1 + cliprange_high)
    clip_ratio_lower = clip_ratio_c * torch.ones_like(ratio)

    pg_losses1 = -advantages * ratio
    pg_losses2 = -advantages * clip_ratio_higher
    pg_losses3 = -advantages * clip_ratio_lower

    clip_pg_losses1 = torch.maximum(pg_losses1, pg_losses2)
    gt_mask_1 = torch.gt(pg_losses2, pg_losses1)
    pg_clipfrac = verl_F.masked_mean(gt_mask_1.float(), response_mask)
    clip_ratio_1 = torch.where(gt_mask_1, clip_ratio_higher, ratio)

    clip_pg_losses2 = torch.minimum(pg_losses3, clip_pg_losses1)
    gt_mask_2 = torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0)
    pg_clipfrac_lower = verl_F.masked_mean(gt_mask_2.float(), response_mask)
    clip_ratio_2 = torch.where(gt_mask_2, clip_ratio_lower, clip_ratio_1)

    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1)
    clip_ratio = torch.where(advantages < 0, clip_ratio_2, clip_ratio_1)

    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, ratio, clip_ratio
